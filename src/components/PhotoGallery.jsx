...
import tattooPhoto from '../assets/images/tattoo-photo.jpg';
...
const PhotoGallery = () => {
  ...
  const [showSurprise, setShowSurprise] = useState(false); // Nuevo estado para la sorpresa

  const nextPhoto = () => {
    if (currentIndex < photos.length - 1) {
      setCurrentIndex(currentIndex + 1);
    } else {
      setShowSurprise(true); // Mostrar sorpresa al finalizar
    }
  };
  ...
  const closeSurprise = () => {
    setShowSurprise(false); // Ocultar la sorpresa
  };
    ...
    <div className="photo-gallery">
      <div className="photo-container">
        <img
          ...
          className={`photo ${photos[currentIndex].className || ''}`}
        />
